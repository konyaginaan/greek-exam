...
function renderGrid() {
  ...
  grid.innerHTML = cards.map(([id, d], i) => `
    <div class="vcard" data-id="${id}" onclick="openCard(this)">
      ...
      <div class="card-num">${String(i + 1).padStart(2, '0')} · 2026</div>
    ...
    <div class="dict-header">
      <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px">
        <div style="display:flex;align-items:flex-start;gap:10px">
          <button class="collapse-btn-inline" onclick="closeDict()" title="Свернуть">
            <svg viewBox="0 0 14 14" width="14" height="14" stroke="currentColor" fill="none" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><polyline points="5,3 9,7 5,11"/></svg>
          </button>
          <div>
            <div class="dict-title-gr" style="color:${d.accent}">${d.titleGr}</div>
            <div class="dict-title-ru">${d.titleRu}</div>
            <span class="dict-level" style="color:${d.accent};border-color:${d.accent}">${d.level}</span>
          </div>
        </div>
        <button class="copy-btn" onclick="copyVocab()" title="Скопировать словарь">
          <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="5" y="5" width="9" height="9" rx="2"/><path d="M11 5V3a2 2 0 00-2-2H3a2 2 0 00-2 2v6a2 2 0 002 2h2"/></svg>
        </button>
      </div>
    </div>
    ${d.panels.map(p => `
...
function openCard(el) {
  ...
  document.getElementById('lbGr').textContent = d.titleGr;
  document.getElementById('lbRu').textContent = d.titleRu;
...
function openDict() {
  if (!cur) return;
  document.getElementById('dictScroll').innerHTML = buildDictHTML(VOCAB[cur]);
...
function copyVocab() {
  ...
  navigator.clipboard.writeText(text).then(() => {
    document.querySelectorAll('.copy-btn,.mob-copy-btn').forEach(btn => {
