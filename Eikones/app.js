...
function getCardCategories(d) {
  return Array.isArray(d.category) ? d.category : [d.category];
}
...
function buildFilters() {
  ...
  const usedCats = new Set(tabCards.flatMap(([, d]) => getCardCategories(d)));
  container.innerHTML = '';
...
function renderGrid() {
  ...
  const cards = activeCategory === 'all'
    ? all
    : all.filter(([, d]) => getCardCategories(d).includes(activeCategory));
...
function openCard(el, fromRandom) {
  cur = el.dataset.id;
  const d = VOCAB[cur];
  if (!d) return;
  ...
  const rndBtn = document.getElementById('randomNextBtn');
  if (rndBtn) rndBtn.classList.toggle('visible', !!fromRandom);
}
